 ...
 * ContextUtil.kt is part of Auralis.
 *
...
fun Context.newBroadcastPendingIntent(action: String): PendingIntent =
    PendingIntent.getBroadcast(
        this,
        IntegerTable.REQUEST_CODE,
        // Target this app explicitly so that the PendingIntent can never resolve to a
        // third-party receiver, and flag it registered-only as it only targets the
        // dynamically-registered SystemPlaybackReceiver.
        Intent(action).setPackage(packageName).setFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY),
        PendingIntent.FLAG_IMMUTABLE,
    )
