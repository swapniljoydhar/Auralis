...
import com.auralis.player.music.service.MusicServiceFragment
import com.auralis.player.playback.service.PlaybackServiceFragment
import dagger.hilt.android.AndroidEntryPoint
import javax.inject.Inject
import timber.log.Timber

@AndroidEntryPoint
class AuralisService :
    ...
    override fun onGetRoot(
        ...
        if (!isAuthenticClient(clientPackageName, clientUid)) {
            Timber.w("Rejecting spoofed media-browser package: $clientPackageName")
            return BrowserRoot(EMPTY_BROWSER_ROOT_ID, null)
        }
        return musicFragment.getRoot()
    }

    private fun isAuthenticClient(packageName: String, uid: Int): Boolean {
        return packageManager.getPackagesForUid(uid)?.contains(packageName) == true
    }
    ...
    companion object {
        private const val EMPTY_BROWSER_ROOT_ID = "__auralis_empty_root__"
        const val ACTION_START = BuildConfig.APPLICATION_ID + ".service.START"

        var isForeground = false
            private set

        // This is only meant for Auralis to internally ensure that it's state management will work.
